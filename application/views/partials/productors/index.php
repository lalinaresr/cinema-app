...
<?php if ($productors->num_rows() > 0) : ?>
            ...
            <tbody>
                <?php foreach ($productors->result_array() as $productor) : ?>
                    <tr class="<?= $productor['id_status'] == 1 ? 'success' : 'danger';  ?>">
                        ...
                        <td>
                            <a href="<?= site_url("productors/view/{$productor['id_productor']}"); ?>" class="btn btn-info btn-sm"><span class="glyphicon glyphicon-eye-open"></span></a>
                            <a href="<?= site_url("productors/edit/{$productor['id_productor']}"); ?>" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-pencil"></span></a>
    ...
    <?php
        foreach ($productors->result_array() as $productor) :
            $this->load->view('components/productors/view-logo-modal', compact('productor'));
        endforeach;
    ?>
